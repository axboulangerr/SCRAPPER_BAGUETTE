"""
Handler principal pour la commande LOAD
"""
import sys
...
class LoadHandler(BaseCommand):
    """Handler principal pour toutes les variantes de la commande LOAD"""
    ...
    def _debug_print(self, message: str):
        ...
        if self.debug_mode:
            colored_prefix = CommandColors.colorize_prefix("LOAD", "LOAD")
            print(f"{colored_prefix} {message}")
    
    def _load_subcommands(self):
        """Charge dynamiquement toutes les sous-commandes LOAD disponibles"""
        load_dir = Path(__file__).parent
        
        # Parcourt tous les sous-dossiers de load/
        for subcommand_dir in load_dir.iterdir():
    ...
    def _load_subcommand_module(self, command_file: Path, subcommand: str):
        ...
        try:
            spec = importlib.util.spec_from_file_location(f"load_{subcommand}", command_file)
            module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(module)
            
            # Récupère la classe de commande (convention: LoadSubcommandCommand)
            class_name = f"Load{subcommand.title()}Command"
    ...
    def execute(self, args: List[str], variables: Dict[str, Any]) -> Any:
        """
        Exécute la commande LOAD avec dispatch vers la bonne sous-commande
        ...
        """
        if len(args) < 1:
            raise ValueError("LOAD: Il faut spécifier une sous-commande (ex: LOAD URL \"...\")")
        ...
        if subcommand not in self.subcommands:
            ...
            raise ValueError(f"LOAD: Sous-commande '{subcommand}' inconnue. Disponibles: {available}")
