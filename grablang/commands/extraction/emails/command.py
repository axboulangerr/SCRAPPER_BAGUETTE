"""
Commande EXTRACT EMAILS pour extraire toutes les adresses email d'un contenu HTML ou texte
"""
...
class ExtractionEmailsCommand(BaseCommand):
    ...
    def execute(self, args: List[str], variables: Dict[str, Any]) -> List[str]:
        ...
        # Support des chaînes de texte directement
        if isinstance(last_result, str):
            self._debug_print(f"Extraction des emails depuis une chaîne de texte")
            emails = self._extract_emails_from_text(last_result)
            self._debug_print(f"{len(emails)} email(s) unique(s) trouvé(s)")
            
            # Si debug activé, affiche un aperçu
            if self.debug_mode and emails:
                preview_count = min(5, len(emails))
                self._debug_print(f"  Emails: {', '.join(emails[:preview_count])}")
                if len(emails) > 5:
                    self._debug_print(f"  ... et {len(emails) - 5} autre(s)")
            
            return emails
        
        # Support des éléments HTML
        elements_to_process = []
        if isinstance(last_result, ResultSet):
            ...
            raise ValueError(f"EXTRACT EMAILS: Type de données non supporté: {type(last_result).__name__}. Supporté: str, Tag, ResultSet, BeautifulSoup")
